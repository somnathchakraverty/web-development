 ...
 */
tagsInput.directive('tagsInput', ["$timeout","$rootScope", "$document", "$window", "tagsInputConfig", "tiUtil", function($timeout, $rootScope, $document, $window, tagsInputConfig, tiUtil) {
    function TagList(options, events, onTagAdding, onTagRemoving) {
        ...
        self.remove = function(index) {
            var tag = self.items[index];
                // modify by akash to remove li class on remove tag

                $("a:contains('"+tag.text+"')").closest("li").removeClass("selectedtest");
        
                var rootIndex = -1;

                 if($rootScope.selectData != undefined)
                 {

                    $rootScope.selectData.forEach(function(ele,ind){
                ...
                if(rootIndex != -1)
                {
                    $rootScope.selectData.splice(rootIndex, 1); 
                }
    ...
    return {
        ...
        controller: ["$scope", "$attrs", "$element", function($scope, $attrs, $element) {
            ...
            tagsInputConfig.load('tagsInput', $scope, $attrs, {
                ...
                placeholder: [String, 'Find your tests/packages'],
            ...
            // modify by akash , false replace spaces with dashes
            ...
            this.registerAutocomplete = function() {
                ...
                return {
                    addTag: function(tag) {
                        console.log("nginput tag add ",tag);
                        $rootScope.addTags(tag);
...
tagsInput.directive('autoComplete', ["$document", "$timeout", "$sce", "$q", "tagsInputConfig", "tiUtil", function($document, $timeout, $sce, $q, tagsInputConfig, tiUtil) {
    function SuggestionList(loadFn, options, events) {
        ...
        self.load = tiUtil.debounce(function(query, tags) {
            ...
            promise.then(function(items) {
                ...
                // modify by akash for create data
                items = items.data ;
    ...
    return {
        ...
        controller: ["$scope", "$element", "$attrs", function($scope, $element, $attrs) {
            ...
            // change min length of search , modify by akash 
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            scope.addSuggestion = function() {
                ...
                if (suggestionList.selected) {
                    // modified by akash add class to populartest and popular package at the time of searching test 
                    $(".populartests li:contains('"+suggestionList.selected.text+"')").closest("li").addClass("selectedtest");
                    // end
...
tagsInput.run(["$templateCache", function($templateCache) {
     ...
     // create modification by akash , replace track keyword in ng-repeat
  ...
  //  $templateCache.put('ngTagsInput/auto-complete.html',
  //   "<div class=\"autocomplete\" ng-show=\"suggestionList.visible\"><ul class=\"suggestion-list\"><li class=\"suggestion-item\" ng-repeat=\"item in suggestionList.items track by $index\" ng-class=\"{selected: item == suggestionList.selected}\" ng-click=\"addSuggestionByIndex($index)\" ng-mouseenter=\"suggestionList.select($index)\"><ti-autocomplete-match data=\"::item\"></ti-autocomplete-match></li></ul></div>"
  // );
